...
import worker, { API_VERSION, RESPONSE_CONTRACT } from './wpa-protocol-bot-v35.1.2-academic-entrypoint.mjs';
...
test('no-context metadata is normalized',()=>{const data=worker.__test?.normalizePayload?worker.__test.normalizePayload({mode:'none',servedBy:'no-context',hasContext:true,sources:[],answer:'Не видам доволно контекст.'}):null;assert.equal(data,null);});
