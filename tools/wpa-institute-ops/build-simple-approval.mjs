...
const releasePolicy = await readJson(path.join(ROOT, 'data/wpa-24x7-release-gate-policy.json'), {});
...
const approvalQueue = {
  ...
  release_gate_policy: '/data/wpa-24x7-release-gate-policy.json',
  owner_command: releasePolicy.owner_command?.canonical_phrase_mk || 'ОДОБРУВАМ',
  core_rule_mk: cognitive.core_rule_mk || releasePolicy.core_rule_mk || 'WPA НЕ ГО ПРЕЗЕМА ТУЃОТО ЗНАЕЊЕ. WPA ГО ПРОВЕРУВА, ГО РАЗЛОЖУВА, ГО СООЧУВА СО СОПСТВЕНАТА ДОКТРИНА, ГО МЕРИ, ГО ПРЕИСПИТУВА И ОД НЕГО СОЗДАВА НОВА WPA ИНСТИТУЦИОНАЛНА ВРЕДНОСТ.',
  owner_gate: {
    ...
    green_cycle_required: 'YES_CURRENT_PR_HEAD',
    approval_method: 'Explicit owner approval followed by immediate re-verification of the current PR head/checks and protected production application; external distribution remains separately scoped.',
...
await fs.writeFile(path.join(OUT, 'system-map.json'), JSON.stringify({ generated, cognitive_flow: cognitiveFlow, operating_flow: simpleFlow, release_gate_policy: approvalQueue.release_gate_policy }, null, 2), 'utf8');
