...
const normalizeLicense = (value) => clean(value).toLowerCase();
...
function reusableOpenLicense(value) {
  ...
  if (/\b(?:nc|nd|sa)\b|by[-_ ]?nc|by[-_ ]?nd|by[-_ ]?sa|creativecommons\.org\/licenses\/by-(?:nc|nd|sa)/i.test(license)) return false;
  if (/^cc[-_ ]?by(?:[-_ ]?\d(?:\.\d)?)?$/i.test(license)) return true;
  if (/creativecommons\.org\/licenses\/by\//i.test(license)) return true;
  return false;
}
...
function hardenScholarlyAtom(atom) {
  ...
  const reason = originalAccessBasis === 'OPEN_LICENSE'
    ? 'Licence is NC, ND, SA, unknown, or otherwise not approved for broad WPA-native adaptation.'
...
function evaluateRobots(groups, targetUrl) {
  ...
  for (const group of selected) {
    ...
    for (const rule of group.rules || []) {
      ...
      try {
        if (ruleRegex(rule.pattern).test(path)) matches.push({ ...rule, length: rule.pattern.replace(/[*$]/g, '').length });
      } catch {}
    }
  }
...
async function robotsDecision(targetUrl) {
  let url;
  try { url = new URL(targetUrl); } catch { return { decision: 'HOLD', status: 'INVALID_URL', url: targetUrl }; }
  if (!['http:', 'https:'].includes(url.protocol)) return { decision: 'HOLD', status: 'NON_PUBLIC_PROTOCOL', url: targetUrl };
  const originPolicy = await fetchRobotsOrigin(url.origin);
  if (originPolicy.fail_closed_hold) return { decision: 'HOLD', status: originPolicy.status, url: targetUrl, error: originPolicy.error || null };
...
async function hardenInstitutionalAtoms(atoms) {
  ...
  for (const atom of atoms || []) {
    const refs = (atom.source_refs || []).map((ref) => ref?.url_or_identifier).filter((value) => /^https?:\/\//i.test(String(value || '')));
    const decisions = [];
    ...
    if (blocked || atom.portal_signals?.source_body_retained !== false) {
      quarantine.push({
        atom_id: atom.atom_id,
        institution_id: atom.institution_id,
        institution_name: atom.institution_name,
        release_state: 'WITHHELD_RIGHTS_SOURCE_CHECK',
        reason: refs.length === 0 ? 'No public HTTP(S) source reference is available.' : (atom.portal_signals?.source_body_retained !== false ? 'Source-body retention invariant failed.' : 'At least one source path is disallowed or could not be safely verified against robots.txt.'),
...
await writeJson(SCHOLARLY_FILE, { ...scholarly, generated, rights_source_gate_mode: 'FAIL_CLOSED', atoms: hardenedScholarly });
...
const report = {
  schema: 'wpa-rights-source-gate/1.1',
  generated,
  mode: 'FAIL_CLOSED',
  purpose: 'Independent post-harvest rights and exact-path robots verification before WPA-native transformation.',
  scholarly: {
  ...
  controls: {
    ...
    cc_nc_nd_sa_are_not_routed_for_broad_adaptation: true,
