...
const status = {
  ...
  systems: {
    product_factory: {
      status: Array.isArray(productManifest.products) && productManifest.products.length >= 5 ? 'OPERATIONAL_PRODUCT_BUILD' : 'BUILD_INCOMPLETE',
...
<header><div><span class="tag">WPA INSTITUTE · CONTINUOUS GOVERNED OPERATIONS</span><h1>WPA Institute 24/7 Operations</h1><p class="lead">Единствен оперативен слој за производната фабрика, Sublimate Engine, глобалната институционална доказна програма и Pilot 20. Автоматизацијата работи континуирано; канонски промени, научно објавување и институционални одлуки остануваат човечки контролирани.</p><div class="meta">Последен циклус: ${esc(generated)} · cadence: ${esc(status.cadence)}</div></div></header>
<main><div class="numbers"><span class="pill">${esc(sys.global_institutions.total_records)} институционални записи</span><span class="pill">${esc(sys.global_institutions.cumulative_reachability_observations)} reachability observations</span><span class="pill">${esc(sys.global_institutions.human_review_candidates)} human-review candidates</span><span class="pill">${esc((sys.product_factory.product_families||[]).length)} product families</span></div><section class="grid">
...
${card('Journal / Academic / Watch Pipeline', 'CONNECTED TO PRODUCT FACTORY', 'WPA Watch, Academic Search и Journal Watch се влезни слоеви; готовите work-products излегуваат преку Product Factory.', '/analytical-center.html', 'Отвори Analytical Center')}
</section><section class="guard"><strong>Human Authority guardrail</strong><ul>${status.guardrails.map((x)=>`<li>${esc(x)}</li>`).join('')}</ul><p><a href="/operations/institute-24x7-status.json">Machine-readable status JSON</a> · <a href="/data/global-institutions/operations/verification-candidates.json">Institutional verification candidate queue</a></p></section></main></body></html>`;
await fs.writeFile(path.join(OUT, 'index.html'), html, 'utf8');

console.log(JSON.stringify({ generated, checked: checked.length, observations: observations.length, candidates: verificationCandidates.length, sublimate: workerRuntimeClass, products: (productManifest.products || []).length }, null, 2));
