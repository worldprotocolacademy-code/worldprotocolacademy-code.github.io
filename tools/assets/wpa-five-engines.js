...
(function () {
  ...
  const I18N = {
    ...
    "map.desc": { mk: "Едукативен атлас на 161 записи од WPA Master List REV4 — академии, think tank-ови, УН тела, судови и финансиски институции, групирани по континент и категорија.", en: "An educational atlas of 162 records from the WPA Master List REV4 — academies, think tanks, UN bodies, courts, and financial institutions, grouped by continent and category." },
  ...
  /* ------------------------------------------------------------------
     4. Global Protocol Reference Map — WPA Master List REV4 (162 records)
     ------------------------------------------------------------------ */
  ...
  function initReferenceMap() {
    ...
    fetch("data/institutions-master-rev4.json")
      .then((r) => r.json())
      .then((json) => {
      ...
      .catch(() => {
        mount.innerHTML = `<p>${t("Податоците не можеа да се вчитаат. Провери дали data/institutions-master-rev4.json е поставен на истата патека.", "Data could not be loaded. Check that data/institutions-master-rev4.json is deployed at the matching path.")}</p>`;
      });
  }
