#!/usr/bin/env python3
"""Fail-closed creation and verification of a filtered WPA v3 AI Search candidate."""
...
POLL = int(os.getenv("AI_SEARCH_FILTERED_POLL_SECONDS", "15"))
TIMEOUT = int(os.getenv("AI_SEARCH_FILTERED_TIMEOUT_SECONDS", "7200"))
QUERIES = (
...
def manifest(items: list[dict[str, Any]], label: str, filter_prefix: bool) -> dict[str, Any]:
    selected = [i for i in items if not filter_prefix or key(i).startswith(PREFIX)]
    keys = [key(i) for i in selected]
    if any(not k for k in keys):
        raise GuardError(f"{label} contains an empty key")
    if len(set(keys)) != len(keys):
        raise GuardError(f"{label} contains duplicate keys")
    outside = sorted(k for k in keys if not k.startswith(PREFIX))
    non_indexed = sorted(({"key": key(i), "status": status(i)} for i in selected if status(i) not in INDEXED), key=lambda r: r["key"])
    result = {"label": label, "prefix": PREFIX, "count": len(keys), "sha256": digest(keys), "outside_prefix": outside, "non_indexed": non_indexed, "keys": sorted(keys)}
    if len(keys) != EXPECTED:
        raise GuardError(f"{label} count mismatch: expected={EXPECTED} actual={len(keys)}")
    if outside:
        raise GuardError(f"{label} has {len(outside)} keys outside locked prefix")
    if non_indexed:
        raise GuardError(f"{label} has {len(non_indexed)} non-indexed keys; first={non_indexed[:5]}")
    return result

def source_manifest(account: str, token: str) -> dict[str, Any]:
    return manifest(all_items(account, token, SOURCE), "source-protocol-ai-v3", True)

def target_manifest(account: str, token: str) -> dict[str, Any]:
    return manifest(all_items(account, token, TARGET), "target-protocol-ai-v3-filtered", False)

def token_id(source: dict[str, Any]) -> str:
    value = str(source.get("token_id") or "")
    if not UUID_RE.fullmatch(value):
        raise GuardError("protocol-ai does not expose a valid registered AI Search service token_id")
    return value

def verify_target(value: dict[str, Any], service_token_id: str) -> None:
    if value.get("id") not in (None, TARGET) or value.get("type") != "r2" or value.get("source") != BUCKET:
        raise GuardError(f"unexpected target shape: {value}")
    if str(value.get("token_id") or "") != service_token_id:
    ...
    if sorted(source_params.get("include_items") or []) != INCLUDE:
        raise GuardError(f"target include_items mismatch: {source_params}")
    if sorted(source_params.get("exclude_items") or []):
        raise GuardError(f"target unexpectedly has exclude_items: {source_params}")

def source_and_token(account: str, token: str) -> tuple[dict[str, Any], str]:
    value = instance(account, token, SOURCE)
    if value is None or value.get("type") != "r2" or value.get("source") != BUCKET:
        raise GuardError("protocol-ai is not the expected protocol-kb R2 instance")
    return value, token_id(value)

def create_target(account: str, token: str, service_token_id: str) -> dict[str, Any]:
    payload = req("POST", f"{API}/accounts/{account}/ai-search/instances", token, json={
        "id": TARGET,
        "type": "r2",
        "source": BUCKET,
        "token_id": service_token_id,
        "source_params": {"include_items": INCLUDE, "r2_jurisdiction": "default"},
    })
    value = dict((payload or {}).get("result") or {})
    verify_target(value, service_token_id)
    return value
...
def verify_zero(value: dict[str, Any]) -> None:
    ...
    if actual != (EXPECTED, 0, 0, 0, 0):
        raise GuardError(f"target stats mismatch: expected={EXPECTED}/0/0/0/0 actual={'/'.join(map(str, actual))}")
...
def compare(reference: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    if not result["exact_match"]:
        raise GuardError(f"key-set mismatch: missing={len(missing)} unexpected={len(unexpected)}")
    return result
...
def plan(account: str, token: str, out: Path) -> None:
    source, service_token_id = source_and_token(account, token)
    reference = source_manifest(account, token)
    target = instance(account, token, TARGET)
    if target is not None:
        verify_target(target, service_token_id)
    dump(out / "source-instance.json", source)
    dump(out / "approved-source-manifest.json", reference)
    result = {"status": "planned", "target": TARGET, "include_items": INCLUDE, "expected": EXPECTED, "approved_manifest_sha256": reference["sha256"], "target_state": "absent" if target is None else "present-and-shape-verified", "quarantined_instance": "protocol-ai-v3-clean"}
    dump(out / "plan.json", result)
    print(json.dumps(result, ensure_ascii=False, indent=2))

def observe(account: str, token: str, out: Path) -> None:
    _, service_token_id = source_and_token(account, token)
    reference = source_manifest(account, token)
    target = instance(account, token, TARGET)
    if target is None:
        result = {"status": "absent", "target": TARGET, "source_manifest_sha256": reference["sha256"]}
    else:
        verify_target(target, service_token_id)
        target_stats = stats(account, token, TARGET)
        result = {"status": "observed", "target": target, "stats": target_stats, "source_manifest_sha256": reference["sha256"]}
        if int(target_stats.get("queued") or 0) == 0 and int(target_stats.get("running") or 0) == 0:
            try:
                verify_zero(target_stats)
                result["manifest_comparison"] = compare(reference, target_manifest(account, token))
...
def apply(account: str, token: str, approval: str, approved_sha: str, out: Path) -> None:
    if approval != "APPROVE_CREATE_FILTERED_V3" or not SHA_RE.fullmatch(approved_sha):
        raise GuardError("APPLY requires explicit filtered-v3 approval and the PLAN manifest SHA-256")
    source, service_token_id = source_and_token(account, token)
    reference = source_manifest(account, token)
    if reference["sha256"] != approved_sha:
        raise GuardError(f"approved manifest mismatch: approved={approved_sha} current={reference['sha256']}")
    target = instance(account, token, TARGET)
    created = target is None
    target = create_target(account, token, service_token_id) if target is None else target
    verify_target(target, service_token_id)
    dump(out / "target-instance.json", target)
    if created:
        time.sleep(15)
    target_stats = stats(account, token, TARGET)
    active = [j for j in jobs(account, token, TARGET) if not j.get("ended_at")]
    triggered = None
    if not active and (int(target_stats.get("completed") or 0), int(target_stats.get("error") or target_stats.get("errors") or 0)) != (EXPECTED, 0):
        triggered = dict((req("POST", url(account, TARGET, "/jobs"), token, json={"description": "WPA filtered v3 80cd2f51"}) or {}).get("result") or {})
    ...
    while True:
        ...
        if current is None:
            raise GuardError("target disappeared during APPLY")
        verify_target(current, service_token_id)
        target_stats = stats(account, token, TARGET)
        active = [j for j in jobs(account, token, TARGET) if not j.get("ended_at")]
        ...
        if not active and snap["queued"] == 0 and snap["running"] == 0:
            break
        if time.monotonic() >= deadline:
            dump(out / "poll-snapshots.json", snapshots)
            raise GuardError(f"target indexing timeout after {TIMEOUT} seconds")
        time.sleep(POLL)
    dump(out / "poll-snapshots.json", snapshots)
    dump(out / "final-stats.json", target_stats)
    verify_zero(target_stats)
    after = source_manifest(account, token)
    if after["sha256"] != approved_sha:
        raise GuardError(f"source manifest changed during APPLY: approved={approved_sha} after={after['sha256']}")
    candidate = target_manifest(account, token)
    comparison = compare(reference, candidate)
    tests = smoke(account, token)
    result = {"status": "filtered_v3_verified", "created": created, "triggered_job": triggered, "approved_manifest_sha256": approved_sha, "stats": target_stats, "manifest_comparison": comparison, "smoke_tests": tests, "quarantined_instance_untouched": "protocol-ai-v3-clean", "source_instance_untouched": SOURCE, "r2_objects_untouched": True}
...
def self_test() -> None:
    ...
    try:
        EXPECTED = 2
        source = [{"key": PREFIX + "b.json", "status": "indexed"}, {"key": PREFIX + "a.pdf", "status": "completed"}, {"key": "legacy/x.pdf", "status": "error"}]
        target = [{"key": PREFIX + "a.pdf", "status": "indexed"}, {"key": PREFIX + "b.json", "status": "indexed"}]
        assert compare(manifest(source, "source-test", True), manifest(target, "target-test", False))["exact_match"]
    finally:
        EXPECTED = old
    print("guarded filtered candidate self-test: OK")
...
def main() -> None:
    ...
    parser.add_argument("--out", default="/tmp/ai-search-v3-filtered")
    ...
    try:
        if args.mode == "PLAN":
            ...
            apply(account, token, args.approval, args.approved_manifest_sha256.strip().lower(), out)
    except Exception as exc:
