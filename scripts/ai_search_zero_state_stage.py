#!/usr/bin/env python3
import argparse,base64,csv,hashlib,json,mimetypes,os,subprocess,tempfile
from collections import Counter
from pathlib import Path,PurePosixPath
import requests
API='https://api.cloudflare.com/client/v4'; READY='__ai_search_ready_v1__'; LIMIT=3700000
...
def sh(cmd,cap=False):
 print('+',' '.join(map(str,cmd)),flush=True); return subprocess.run(cmd,check=True,text=True,capture_output=cap)
...
class CF:
 ...
 def items(self):
  out=[]; p=1
  while 1:
   b=self.api(f'/items?page={p}&per_page=50'); out+=b
   if len(b)<50:return out
   p+=1
 def env(self):
  e=os.environ.copy(); e['CLOUDFLARE_API_TOKEN']=self.r2; e['CLOUDFLARE_ACCOUNT_ID']=self.a; return e
 def get(self,k,p): p.parent.mkdir(parents=True,exist_ok=True); sh([self.w,'r2','object','get',f'{self.b}/{k}','--remote','--file',p],False)
 def put(self,k,p,ct): sh([self.w,'r2','object','put',f'{self.b}/{k}','--remote','--force','--file',p,'--content-type',ct],False)
...
def plan(cf):
 ...
 rows.sort(key=lambda r:(r['action'],r['key'],r['id'])); fp=hbytes(canon(rows)); prefix=f'{READY}/{fp[:20]}/'; sp=inst.get('source_params') or {}
 return {'schema':'1.0','instance':cf.i,'target_prefix':prefix,'fingerprint':fp,'blocked':sum(r['action']=='BLOCK' for r in rows),'path_filters':{'include_items':sp.get('include_items') or [],'exclude_items':sp.get('exclude_items') or [],'prefix':sp.get('prefix'),'r2_jurisdiction':sp.get('r2_jurisdiction') or 'default'},'items':rows}
def ctype(p): return {'.pdf':'application/pdf','.json':'application/json','.txt':'text/plain; charset=utf-8','.md':'text/markdown; charset=utf-8'}.get(p.suffix.lower()) or mimetypes.guess_type(p.name)[0] or 'application/octet-stream'
...
def splitpdf(cf,src,source,prefix,w,verify,m):
 ...
 while start<=pages:
  ...
  while 1:
   ...
   out.unlink()
   if span==1: raise RuntimeError(f'single PDF page exceeds limit: {source} p{start}')
   span=max(1,span//2)
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--mode',choices=['PLAN','STAGE'],required=True); ap.add_argument('--approved-sha',default=''); ap.add_argument('--out',type=Path,required=True); args=ap.parse_args(); args.out.mkdir(parents=True,exist_ok=True); cf=CF(); p=plan(cf); raw=canon(p); sha=hbytes(raw); (args.out/'migration-plan.json').write_bytes(raw); (args.out/'migration-plan.sha256').write_text(sha+'\n'); counts={'total':len(p['items']),'blocked':p['blocked'],'target_prefix':p['target_prefix'],'sha256':sha,'actions':dict(Counter(x['action'] for x in p['items']))}; (args.out/'counts.json').write_text(json.dumps(counts,indent=2)+'\n'); print(json.dumps(counts,indent=2))
