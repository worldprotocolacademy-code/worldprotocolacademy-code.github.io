...
function workingPapers(){
  ...
  if(currentPath!=='/working-papers'&&currentPath!=='/working-papers/index.html')return;
  try{
  ...
  if(grid){
    Array.prototype.slice.call(document.querySelectorAll('#wp013')).forEach(function(node){if(!grid.contains(node))node.remove();});
...
function metrics(){
  if(path()!=='/wpa-metrics-status.html')return;
  var z=document.getElementById('zenodoTotal'),w=document.getElementById('wpTotal'),f=document.getElementById('zenodoFormula');
  if(z)z.textContent='24';
  if(w)w.textContent='14';
  if(f)f.textContent='24 = 14 Working Papers + 9 Protocol Notes + 1 Global Strategic Plan.';
}
