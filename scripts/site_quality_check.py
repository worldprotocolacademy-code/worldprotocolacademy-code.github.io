...
ALLOWED_SITEMAP_PATHS = {
    ...
    "/working-papers/",
    "/journal/",
    "/journal/editorial-governance-status.html",
    "/journal/vol-1-issue-1-2026.html",
    "/papers.html",
    "/scholar/wpa-wp-009.html",
    "/programmes.html",
    "/certification.html",
    "/professional-english.html",
    ...
    "/partnerships/",
    "/student-desk/",
    "/institutional-enquiry.html",
    "/strategy/master-strategy-2026-2030.html",
    "/wpa-institutional-maturity-roadmap.html",
    "/languages/",
    ...
    "/wpa-global-ranking-roadmap.html",
    "/virtual-sande-ai.html",
    "/protocolometry-center.html",
    "/wpa-trust-layer.html",
    "/wpa-scenario-film-lab.html",
    "/wpa-metrics-status.html",
    "/wpa-protocol-stress-test.html",
    "/wpa-protocol-knowledge-check.html",
    "/wpa-briefings.html",
    "/wpa-services.html",
    "/wpa-one-page-service-profile.html",
...
def check_sitemap(errors: list[str]) -> None:
    ...
    locs = [n.text.strip() for n in tree.findall(".//sm:loc", ns) if n.text and n.text.strip()]
    if not locs:
...
def check_robots(errors: list[str]) -> None:
    ...
    for group in parse_robots_groups(text):
        ...
        rules = {r.lower().replace(" ", "") for r in group.get("rules", [])}
        if "disallow:/" in rules and ("*" in agents or agents & (PUBLIC_SEARCH_USER_AGENTS - {"*"})):
            add_error(errors, "robots.txt blocks a public search crawler with Disallow: /")
        if "disallow:/wpaws" in {r.rstrip("/") for r in rules} and ("*" in agents or agents & PUBLIC_SEARCH_USER_AGENTS):
            add_error(errors, "robots.txt should not block /wpaws/ for public crawlers; use noindex instead")
...
def check_privacy_hotfixes(errors: list[str]) -> None:
    ...
    if student_desk.exists() and "noindex" not in read_text(student_desk).lower():
